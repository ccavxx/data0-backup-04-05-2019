...
var Cg_ded_end_date = '01/04/2019';
...
try{
		...
		}else if(dueDate234A == '17/10/2016'){
			dueDate234A = '18/10/2016';
			
		}else if(dueDate234A == '30/11/2016'){
			dueDate234A = '01/12/2016';
		}
...
function calcTotScheduleAMTC(){
    try{
        ...
        var totAmtcCredGrossCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[2].amtCreditFwd')[0];
        var totAmtcCredBFCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[2].amtCreditBalBroughtFwd')[0];
        var totAmtcCredUtilCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[2].amtCreditUtilized')[0];
        var totBalAmtcCredCFCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[2].balAmtCreditCarryFwd')[0];
        ...
        document.getElementsByName('itrScheduleAMTC.totAmtCreditUtilisedCY')[0].value=eval(parseInt(coalesce(totAmtcCredUtilPY1.value),10))+eval(parseInt(coalesce(totAmtcCredUtilPY2.value),10))+eval(parseInt(coalesce(totAmtcCredUtilCY.value),10));
        
        balAmtcCreditCarriedFwdCY.value = totAmtcCredGross.value;
	
        document.getElementsByName('itrScheduleAMTC.totAMTGross')[0].value=eval(parseInt(coalesce(totAmtcCredGross.value,10))) + eval(coalesce(parseInt(totAmtcCredGrossPY1.value,10))) + eval(coalesce(parseInt(totAmtcCredGrossPY2.value,10)))+ eval(coalesce(parseInt(totAmtcCredGrossCY.value,10)));
        document.getElementsByName('itrScheduleAMTC.totSetOffEys')[0].value=eval(parseInt(coalesce(totAmtcCredSetOffPY1.value,10))) + eval(parseInt(coalesce(totAmtcCredSetOffPY2.value,10)));
        document.getElementsByName('itrScheduleAMTC.totBalBF')[0].value=eval(parseInt(coalesce(totAmtcCredBFPY1.value,10))) +eval(parseInt(coalesce(totAmtcCredBFPY2.value,10))) +eval(parseInt(coalesce(totAmtcCredBFCY.value,10))) ;
        
        document.getElementsByName('itrScheduleAMTC.totBalAMTCreditCF')[0].value=eval(parseInt(coalesce(totBalAmtcCredCFPY1.value,10))) + eval(parseInt(coalesce(totBalAmtcCredCFPY2.value,10)))+ eval(coalesce(parseInt(balAmtcCreditCarriedFwdCY.value,10))) + eval(parseInt(coalesce(totBalAmtcCredCFCY.value,10)));
	...
	if(status =='I'){	
		...
		if(status =='I'){
			if((resStatus== 'RES' || resStatus== 'NOR') && age > eval('59')){
				...
				if(gtiLimit > parseInt('60000' ,10)){
				  if( parseInt(coalesce(sec80D.value),10) > eval('60000')){
				...
				if(gtiLimit > parseInt('55000' ,10)){
				if( parseInt(coalesce(sec80D.value),10) > eval('55000')){
...
//80QQB
	...
	sumDeductionsWithout80GG(sec80CSysCal,sec80CCCSysCal,sec80CCDiSysCal,sec80CCD1BSysCal,sec80CCDiiSysCal,
			sec80DSysCal,sec80DDSysCal,sec80DDBSysCal,sec80ESysCal,sec80GSysCal,sec80GGCSysCal,
	...
	sumDeductions(sec80CSysCal,sec80CCCSysCal,sec80CCDiSysCal,sec80CCD1BSysCal,sec80CCDiiSysCal,
			sec80DSysCal,sec80DDSysCal,sec80DDBSysCal,sec80ESysCal,sec80GSysCal,sec80GGSysCal,sec80GGCSysCal,
			sec80IASysCal,sec80IABSysCal,sec80IBSysCal,sec80ICSysCal,sec80IDSysCal,sec80JJASysCal,sec80JJAASysCal,
			sec80USysCal,sec80CCGSysCal,sec80RRBSysCal,sec80QQBSysCal,sec80TTASysCal);
...
function sumDeductions(sec80CSysCalc,sec80CCCSysCalc,sec80CCDiSysCalc,sec80CCD1BSysCalc,sec80CCDiiSysCalc,
	sec80DSysCalc,sec80DDSysCalc,sec80DDBSysCalc,sec80ESysCalc,sec80GSysCalc,sec80GGSysCalc,sec80GGCSysCalc,
	sec80IASysCal,sec80IABSysCal,sec80IBSysCal,sec80ICSysCal,sec80IDSysCal,sec80JJASysCal,sec80JJAASysCal,
...
function setValuesCG54B() {
	...
	if (yearConstant == "" && amountUtilized == "" && amountUnUtilized == "") {
		...
		document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.unUtilizedCapGain54B.year')[0].value = '2013-14';
	}
	...
	if (yearConstantStcg == "" && amountUtilizedStcg == "" && amountUnUtilizedStcg == "") {
		...
		document.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.unUtilizedCapGain54B.year')[0].value = '2013-14';
	}
...
function checkNoOfRowsFilled() {
	...
	if (noOfBankAccounts != '' && (noOfBankAccounts - 1) != count) {
		j.setFieldError('itr.scheduleBA.noOfBankAcc','Number of bank accounts held during the previous year should be equal to number of rows entered in the below tables');
	}
}
...
function surchargeEditable(){
	...
	if(splRateIncTax>0){
		document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.surcharge')[0].readOnly = false;
		document.getElementsByName('partBTTI.computationOfTaxLiability.educationCess')[0].readOnly = false;
	}else{
		document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.surcharge')[0].readOnly = true;
