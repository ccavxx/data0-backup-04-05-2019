...
var Filing_dueDate = '31/07/2018';
var Int_start_date_234A = '01/08/2018';
...
var Int_234A_Due_date_234A = '01/08/2018'; // Modified due to the due date
// extended for filing the return
//var Int_234A_IF_Yes_dt = '01/10/2018';
//var Int_234A_IF_Yes_dt_SAT = '30/09/2018';
var Int_234A_IF_NO_dt = '31/07/2018'; // Modified due to the due date extended
...
function enableTableForSingleRow(element, value, tableId) {
...
function calSchS(tableId) {
	try {
		...
		for (var i = 0; i < salaryCount; i++) {
			...
			salaryTotal = eval(parseInt(coalesce(salaryTotal), 10)
						+parseInt(coalesce(salary.value), 10)
			...
			var employerCategory = document.getElementsByName('partAGEN1.personalInfo.employerCategory')[0].value;
			
			if(employerCategory == 'GOV' || employerCategory == 'PSU')
			 {
			...
			}else if(employerCategory == 'OTH')
				{
				if ((parseInt(salaryTotal, 0) < parseInt(
						coalesce(deductionUS16.value), 10))
						|| parseInt(coalesce(deductionUS16.value), 10) > 5000) {
					addErrorXHTML(document.getElementsByName('scheduleS.salaries['
							+ i + '].salarys.deductionUnderSection16')[0],
							'Deduction u/s 16 should not exceed '
									+ Math.min(5000, zeroOrMore(salaryTotal)), true);
					document.getElementsByName('scheduleS.salaries[' + i
							+ '].salarys.deductionUnderSection16')[0].value = "0";
				 }
			}else if(employerCategory == 'NA')
				{
				if ((parseInt(salaryTotal, 0) < parseInt(
						coalesce(deductionUS16.value), 10))
						|| parseInt(coalesce(deductionUS16.value), 10) > 0) {
					addErrorXHTML(document.getElementsByName('scheduleS.salaries['
							+ i + '].salarys.deductionUnderSection16')[0],
							'Deduction u/s 16 should not exceed '
									+ Math.min(0, zeroOrMore(salaryTotal)), true);
					document.getElementsByName('scheduleS.salaries[' + i
							+ '].salarys.deductionUnderSection16')[0].value = "0";
				 }
			}
...
function checkUnUtilizedCG() {
	...
	if (UnUtilizedFlag == 'Y') {
		 tab1 = document.getElementById('scheduleStcgunUtilizedCapGain54');
		 isRowBlank = checkRowBlank('scheduleStcgunUtilizedCapGain54', 2, 1);
...
function checkUnUtilizedLTCG() {
	...
	if (UnUtilizedFlag == 'Y') {
		...
		var isRowBlank = checkRowBlank('scheduleLtcgunUtilizedCapGain54', 2, 1);
		if (isRowBlank) {
			j
					.setFieldError(
							'scheduleCGPost45.LongTermCapGainPost45.unutilizedCapgainFlag',
...
function caclDedSchVIA() {
	try {
		...
		var sec80DASysCal = document
		.getElementsByName('scheduleVIA.usrDeductUndChapVIA.section80DHealthInsPremium.healthInsurancePremiumHiddenValue')[0];

		var sec80DBSysCal = document
		.getElementsByName('scheduleVIA.usrDeductUndChapVIA.section80DHealthInsPremium.medicalExpenditureHiddenValue')[0];

		var sec80DCSysCal = document
		.getElementsByName('scheduleVIA.usrDeductUndChapVIA.section80DHealthInsPremium.preventiveHealthCheckUpHiddenValue')[0];
		...
		var temp = Math.round(eval(parseInt(coalesce(salaryTotal), 10))
				* eval('0.10'));
		if (status == 'I') {
		...
		var salUppLimit = Math
				.round(eval(parseInt(coalesce(salaryTotal), 10))
						* eval('0.10'));
		...
		var totGrossOS = document
		.getElementsByName('scheduleOS.totalOSGross')[0].value;
		
		/*if (status == 'I') {

			if (resStatus == 'RES' || resStatus == 'NOR') {
				if (parseInt(coalesce(gtiLimit), 10) > eval('300000')) {
				  if(parseInt(coalesce(totGrossOS), 10) > eval('300000')) {
					...
					if ((parseInt(coalesce(sec80QQB.value), 10) > parseInt(
							coalesce(gtiLimit), 10)) || (parseInt(coalesce(sec80QQB.value), 10) > parseInt(
									coalesce(totGrossOS), 10))) {
						sec80QQBSysCal.value = Math.min(parseInt(
									coalesce(totGrossOS), 10),parseInt(
							coalesce(gtiLimit), 10));
					} else {
						sec80QQBSysCal.value = parseInt(
								coalesce(sec80QQB.value), 10);
			...
			if (resStatus == 'RES' || resStatus == 'NOR') {
				if (parseInt(coalesce(gtiLimit), 10) > 0) {
					if (parseInt(isNVL(sec80QQB.value), 10) > parseInt('0', 10)) {
						sec80QQBSysCal.value = Math.min(parseInt(totGrossOS, 10), parseInt(
								gtiLimit, 10), 300000, parseInt(isNVL(sec80QQB.value),
								10));
					} else {
						sec80QQBSysCal.value = parseInt('0', 10);
					}
			...
			if (resStatus == 'RES' || resStatus == 'NOR') {
				if (parseInt(coalesce(gtiLimit), 10) > eval('300000')) {
					...
					if ((parseInt(coalesce(sec80RRB.value), 10) > parseInt(
							coalesce(gtiLimit), 10)) || (parseInt(coalesce(sec80RRB.value), 10) > parseInt(
									coalesce(gtiLimit), 10))) {
						sec80RRBSysCal.value = Math.min(parseInt(
								coalesce(totGrossOS), 10),parseInt(
										coalesce(gtiLimit), 10));
					} else {
						sec80RRBSysCal.value = parseInt(
			...
			if (resStatus == 'RES' || resStatus == 'NOR') {
				if (parseInt(coalesce(gtiLimit), 10) > 0) {
					if (parseInt(isNVL(sec80RRB.value), 10) > parseInt('0', 10)) {
						sec80RRBSysCal.value = Math.min(parseInt(totGrossOS, 10), parseInt(
								gtiLimit, 10), 300000, parseInt(isNVL(sec80RRB.value),
								10));
					} else {
						sec80RRBSysCal.value = parseInt('0', 10);
					}
...
function setOtherGrossSourceOnChangeCGSTCG() {
	...
	var allSectionArray = [ 'A1e', 'A2e_111A', 'A2e_115AD', 'A3a', 'A3b',
			'A4e', 'A5e', 'A6'];

	if ((status == 'RES' || status == 'NOR')&& ben115HFlg != 'Y' ) {
		addAllOptionsCGSTCG(allSectionArray, rowCount);

		var sectionArray = [ 'A3a', 'A3b', 'A4e'];
...
function setOtherGrossSourceOnChangeCGLTCG() {
	...
	if ((status == 'RES' || status == 'NOR')&& ben115HFlg != 'Y' ) {
		...
		var sectionArray = [ 'B4c','B5e_21ciii', 'B5e_5AC1c', 'B5e_5ADiii','B6c', 'B6f'];
...
function adjustCG() {
	...
	if (document.getElementsByName('partAGEN1.filingStatus.residentialStatus')[0].value == 'NRI' || ben115HFlg == 'Y'){
		...
		// A4
		document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdRecvUnqshr')[0].readOnly = false;
		document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fairMrktValueUnqshr')[0].readOnly = false;
		//	document
			//	.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdSec50CA')[0].readOnly = false;
		document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdOthUnqshr')[0].readOnly = false;
		document
		...
		var fullValueConsdRecvUnqshr = document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdRecvUnqshr')[0];
		fullValueConsdRecvUnqshr.readOnly = true;
		fullValueConsdRecvUnqshr.value = 0;

		var fairMrktValueUnqshr = document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fairMrktValueUnqshr')[0];
		fairMrktValueUnqshr.readOnly = true;
		fairMrktValueUnqshr.value = 0;

		var fullValueConsdSec50CA = document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdSec50CA')[0];
		fullValueConsdSec50CA.readOnly = true;
		fullValueConsdSec50CA.value = 0;

		var fullValueConsdOthUnqshr = document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdOthUnqshr')[0];
		fullValueConsdOthUnqshr.readOnly = true;
		fullValueConsdOthUnqshr.value = 0;
...
function validateCGSecwiseDed() {
	...
	sumSectionWiseCGTableD('schduleCGDedD1C', arrDed,'54EC');
	sumSectionWiseCGTableD('schduleCGDedD1D', arrDed,'54EE');
	sumSectionWiseCGTableD('schduleCGDedD1E', arrDed,'54F');
	sumSectionWiseCGTableD('schduleCGDedD1F', arrDed,'54GB');
	sumSectionWiseCGTableD('schduleCGDedD1G', arrDed,'115F');
...
//To disable or enable the values on chnage of status on load
function onStatusChangeOnLoad() {
...
function calculateCYLA(cgosIncome) {
	try {
		...
		var hpLossSetOff = coalesceSetRet('scheduleCYLA.totalLossSetOff.totHPlossCurYrSetoff');
		var hpLossRem = coalesceSetRet('scheduleCYLA.lossRemAftSetOff.balHPlossCurYrAftSetoff');

		if((hpLossSetOff+hpLossRem)<=200000){
			...
			var losses =  coalesceSetRet('scheduleCYLA.lossRemAftSetOff.balOthSrcLossNoRaceHorseAftSetoff');
		}
...
function setOffHPLossCYLA(cgosIncome) {
	try {
		...
		if (setOffRem >= 0) {
			...
			if (!checkIfChanged('scheduleCYLA.Salaries.incCYLA.hPlossCurYrSetoff')) {
				document
						.getElementsByName('scheduleCYLA.Salaries.incCYLA.hPlossCurYrSetoff')[0].value = prevSetOffRem;
			} else {
				checkHPeditableValidity(
						'scheduleCYLA.Salaries.incCYLA.hPlossCurYrSetoff',
						'scheduleCYLA.Salaries.incCYLA.incOfCurYrUnderThatHead',
		...
		if (setOffRem >= 0) {
			if (prevSetOffRem == 0) {
				...
				if (!checkIfChanged('scheduleCYLA.othSrcInclRaceHorse.incCYLA.hPlossCurYrSetoff')) {
					document
							.getElementsByName('scheduleCYLA.othSrcInclRaceHorse.incCYLA.hPlossCurYrSetoff')[0].value =
								coalescePath('scheduleCYLA.othSrcInclRaceHorse.incCYLA.incOfCurYrUnderThatHead');
...
function calcCFL(cgosIncome) {
	try {
		...
		if (coalescePath('scheduleOS.incFromOwnHorse.balanceOwnRaceHorse') < 0) {
			...
			setEditableFieldValue(osLoss,Math
...
function calculatePartBTI_first() {
	try {
		...
		shortTermSplRateDTAA.value = coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.stcgUnderDtaa.totAmtStcgUnderDtaa');
		...
		.getElementsByName('partBTI.capGain.longTerm.longTermSplRateDTAA')[0];
		longTermSplRateDTAA.value = coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.ltcgUnderDtaa.totAmtLtcgUnderDtaa');
		...
		totalShortTerm.value = eval(parseInt(shortTerm15Per.value, 10)
				...
				+ parseInt(shortTermAppRate.value, 10) + parseInt(shortTermSplRateDTAA.value, 10));
		// 43b
		...
		totalLongTerm.value = eval(parseInt(longTerm10Per.value, 10)
				+ parseInt(longTerm20Per.value, 10) + parseInt(longTermSplRateDTAA.value, 10));
...
toBeTaxed = zeroOrMore(parseInt(toBeTaxed - exemptionUsed - exmptn10
	- exmptn15 - exmptn20));
		...
		percent5SIInc = parseInt(percent5SIInc, 10)
		+ parseInt(normalInc["5"], 10);
		...
		var surofTaxon1cr = before60 * 0.10; // surcharge on 1 Cr tax
		...
		if ((taxOnTotInc + tempSurcharge) > (taxOnCutOffInc + extraInc + surofTaxon1cr)) {
			...
			var marginalRelief = zeroOrMore(taxOnTotInc + tempSurcharge
					- (taxOnCutOffInc + extraInc + surofTaxon1cr));
			...
			var f_115BBE = eval(parseInt(
					...
					- (parseInt(
							coalesceSetRet('scheduleOS.incChargblSplRateOS.dividendIncome115BBE'),
							10) * 0.60));
			var tempSurcharge = eval((0.10 * parseInt(f_115BBE, 10))
					+ (parseInt(
							coalesceSetRet('scheduleOS.incChargblSplRateOS.dividendIncome115BBE'),
							10) * 0.60 * 0.25));
...
// ===============Interest234F calculation=======================
		...
		var faflag_234F= document.getElementsByName('partBTTI.assetOutIndiaFlag')[0].value;
		var schEIVal= document.getElementsByName('scheduleEI.ltcgWhereSTTPaid')[0].value;
		var lossCFL = document.getElementsByName('partBTI.lossesOfCurrentYearCarriedFwd')[0].value;
		
		var gti_234F =eval(
				parseInt(gti_234FMain,10)
				+ parseInt(schEIVal,10)
			);
		if (faflag_234F == 'YES' || lossCFL > 0) {
			if (filingType == '17' || filingType == '18') {
				if (totInc <= '500000') {
					if (checkFirstDateBefore('01/08/2018', originalFilingDate)) {
						intrst234F = '1000';
					} else {
						intrst234F = '0';
					}
 			...
 			else if (filingType == '11' || filingType == '12' || filingType == '13') {
				if (totInc <= '500000') {
					if (checkFirstDateBefore('01/08/2018', currentDate)) {
						intrst234F = '1000';
					} else {
						intrst234F = '0';
					}
					...
					if (checkFirstDateBefore('01/08/2018', currentDate)
							&& checkFirstDateBefore(currentDate, '31/12/2018')) {
						intrst234F = '5000';
		...
		if(taxPayer == 'I' && (resStatus == 'RES' || resStatus == 'NOR'))
			...
			if (checkFirstDateBefore(dob_234F, '01/04/1938')) {
				if (gti_234F > '500000') {
					if (totInc <= '500000') {
						if (checkFirstDateBefore('01/08/2018',
								originalFilingDate)) {
							intrst234F = '1000';
						} else {
							intrst234F = '0';
...
/*
					...
					if (selfAsspaidDates.length != 0) {
						...
						for (var i = 0; i < selfAsspaidDates.length; i++) {
							...
							intrst234Bprinciple2 = zeroOrMore(eval(parseInt(
									...
									+ parseInt(intrst234Bii, 10) + parseInt(intrst234F ,10)
		...
		var intrstPayUs234F = document.getElementsByName('partBTTI.computationOfTaxLiability.intrstPay.lateFilingFee234F')[0];
		...
		intrstPayable.value = Math.round(eval(parseInt(intrstPayUs234A.value,
				...
				+ parseInt(intrstPayUs234C.value, 10) + parseInt(intrstPayUs234F.value, 10)));
...
function calculate234cIntrst(TDS, TCS, slabs, cgosIncome) {
	try {
		...
		var normalBalIncome = eval(parseInt(coalesceSetRet('partBTI.grossTotalIncome'))
				...
				-parseInt(coalesceSetRet('partBTI.capGain.shortTerm.shortTermSplRateDTAA'))
				-parseInt(coalesceSetRet('partBTI.capGain.longTerm.longTermSplRateDTAA')));
...
function calculateSlabbedTax(netTxblIncome, dontAddExemption) {
	...
	} else if ( status == 'I' && (resStatus == 'RES' || resStatus == 'NOR')
			&& parseInt(age, 10) >= 80) {
		if (parseInt(netTxblIncome, 10) >= parseInt('0', 10)
				&& parseInt(netTxblIncome, 10) <= parseInt('500000', 10)) {
			incTax = parseInt('0', 10);
...
function calculateOS(cgosIncome) {
	...
	totalOSGrossChargblSplRate.value = eval(parseInt(
			...
			+ parseInt(
					...
					10)
			+ parseInt(
					coalesceSetRet('scheduleOS.incChargblSplRateOS.dividendIncome115BBG'),
					10)
			+ parseInt(
	...
	totDeductions.value = eval(parseInt(
			...
			+ parseInt(
					...
					10));
	var amtNotDeductibleUs58 = document
	.getElementsByName('scheduleOS.amtNotDeductibleUs58')[0];

	var profitChargTaxUs59 = document
	.getElementsByName('scheduleOS.profitChargTaxUs59')[0];

	var balanceNoRaceHorse = document
			.getElementsByName('scheduleOS.balanceNoRaceHorse')[0];


	balanceNoRaceHorse.value = eval(parseInt(grossAmtChargblNormalRate.value,10)
			- parseInt(totDeductions.value, 10) + parseInt(amtNotDeductibleUs58.value, 10) + parseInt(profitChargTaxUs59.value, 10));
	...
	balanceOwnRaceHorse.value = eval(parseInt(
			...
			- parseInt(
					...
					10)
			+ parseInt(
					coalesceSetRet('scheduleOS.incFromOwnHorse.amtNotDeductibleUs58'),
					10)
			+ parseInt(
					coalesceSetRet('scheduleOS.incFromOwnHorse.profitChargTaxUs59'),
					10));
...
function calculateOSGross(simap) {
	...
	try {
		...
		var resStatus = document
		.getElementsByName('partAGEN1.filingStatus.residentialStatus')[0];
		...
		var dividendIncome115BBG = document
		.getElementsByName('scheduleOS.incChargblSplRateOS.dividendIncome115BBG')[0];
		...
		total = parseInt(total, 10) - parseInt(temp, 10)  - parseInt(temp562X, 10);
		var allSelects = tabl.getElementsByTagName('SELECT');
		...
		for (var i = 0; i < allSelects.length; i++) {
			...
			if (allSelects[i].value == 'Others') {
				...
				if (allSelects[i].value == '5BBG') {
					...
					dividendIncome115BBG.value = document
							.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['
									+ index + '].sourceAmount')[0].value;
				}
		...
		finalBBDA = dividendIncome115BBDA.value - dropDownBBDA;
		...
		finalBBF = dividendIncome115BBF.value - dropDownBBF;
		...
		finalBBE = dividendIncome115BBE.value - dropDownBBE;
		...
		finalBBG = dividendIncome115BBG.value - dropDownBBG;
		document.getElementsByName('scheduleOS.incChargblSplRateOS.dividendIncome115BBG')[0].value = finalBBG;

		var finalBB = parseInt(0,10);
		finalBB = document.getElementsByName('scheduleOS.incChargblSplRateOS.winningFrmLotteries')[0].value - dropDownBB;
		...
		var temp115BBG = document
				.getElementsByName('scheduleOS.incChargblSplRateOS.dividendIncome115BBG')[0].value;
		...
		secXIIOth.value = eval(sum - temp - tempWinningFromLotteries -temp115BBDA - temp115BBF
				- temp115BBE - temp115BBG - (2*temp562X));
...
/*	if (coalesceSetRet('scheduleCYLA.lossRemAftSetOff.balBusLossAftSetoff') < 0) {
	...
	if (parseInt(
		...
		//coalesceSetRet('scheduleCYLA.lossRemAftSetOff.balHPlossCurYrAftSetoff')
...
function populateSI(cgosIncome, simap) {
	try {
		...
		document.getElementsByName('scheduleSI.splCodeRateTax[1].splRateInc')[0].value = zeroOrMore(parseInt(
			...
			+ parseInt(
					document
					.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.stcgUnderDtaa.totAmtStcgUnderDtaa')[0].value,
			10)
			+ parseInt(
					document
			...
			if (simap[i] != 'undefined' && simap[i] != null && simap[i] != 0) {
				...
				if (!secAdded
						&& !(i == '68' || i == '69' || i == '69A' || i == '69B'
								|| i == '69C' || i == '69D'|| i == 'aggrtvaluewithoutcons562x' || i == 'immovpropwithoutcons562x'
									|| i == 'immovpropinadeqcons562x' || i == 'anyotherpropwithoutcons562x'
									|| i == 'anyotherpropinadeqcons562x' || i == 'Tot562x')) {
...
function getSectionTextMap(key) {
	var sectionTextMap = {
		...
		"Tot562x" : "56(2)(x) - Income of the nature referred to in section 56(2)(x) which is chargeable to tax",
		...
		"DTAAOS" : "Chargeable at DTAA Rate",
        ...
        "B4c":"B4c",
        "B5e_21ciii":"B5e-112(1)(c)",
        "B5e_5AC1c":"B5e-115AC",
        "B5e_5ADiii":"B5e-115AD",
        "B6c":"B6c",
        "B6f":"B6f",
        "B7e":"B7e",
        "B8":"B8",
...
function calCGA6TotalSum() {
	...
	for (var i = 0; i < (noOfRows - 3); i++) {
		var amt = document
				.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.unUtilizedCapGain54B['
...
function calCGPoint9Total(tableId) {
	...
	if (tableId == 'scheduleLtcgDtaa') {
		...
		document
				.getElementsByName('scheduleCGPost45.longTermCapGainPost45.ltcgUnderDtaa.totalAmtNotTaxUsDTAALtcg')[0].value = coalesce(parseInt(
				totalSTCG, 10));
	}
}
...
function mandateB8() {
	...
	var valueB9 = document
			.getElementsByName('scheduleCGPost45.longTermCapGainPost45.ltcgUnderDtaa.totalAmtNotTaxUsDTAALtcg')[0].value;
...
//	var AOPflag = document.getElementsByName('itr2.scheduleAL.interstAOPFlag')[0].value;
	...
	if ((immflag != ""))
	{
		if((jewflag =="")||(archflag =="")||(vehiflag =="") ||(deposflag =="") ||(shreflag =="") ||(insurflag =="") ||(loanflag =="")
			...
			addErrorXHTML('' ,'Please fill the amount fields at Point B or /and Point C. If not applicable, enter zeros.');
			j.setFieldError('itr2.scheduleAL.immovableFlag','Please fill the amount fields at Point B or /and Point C. If not applicable, enter zeros.');}
	...
	if ((jewflag != ""))
	{
		...
		if(liabflag ==""){
			addErrorXHTML('' ,'Please fill the amount field at Point C. If not applicable, enter zeros.');
			j.setFieldError('itr2.scheduleAL','Please fill the amount field at Point C. If not applicable, enter zeros.');
		}
		if(immflag ==""){
	...
	if ((liabflag != ""))
	{
		...
		if((jewflag =="")||(vehiflag =="") ||(deposflag =="") ||(shreflag =="") ||(archflag =="") ||(insurflag =="")
			...
			addErrorXHTML('' ,'Please fill the amount fields at Point B . If not applicable, enter zeros.');
			j.setFieldError('itr2.scheduleAL.immovableFlag','Please fill the amount fields at Point B . If not applicable, enter zeros.');}
...
function enable115HFlag()
{
	...
	if(document.getElementsByName('partAGEN1.filingStatus.benefitUs115HFlg')[0].value == "")
	{
...
function enableBankDtls()
{
	...
	enableField('itr.refund.scheduleBA.bankDtlsFlag','N','itr.refund.scheduleBA.foreignBankDtls[0].ibanCode');
...
function section80DChk()
{
	...
	if( (sec80D == '2' || sec80D == '7' ) && age < 60)
		{
		j.setFieldError('scheduleVIA.usrDeductUndChapVIA.section80DHealthInsPremium.healthInsurancePremium','Please select a valid option from the dropdown provided at Part (A) of Sec.80D under Chapter VI-A');
		addErrorXHTML('','Please select a valid option from the dropdown provided at Part (A) of Sec.80D under Chapter VI-A',true);
		}
	...
	if( (sec80DB == '1' || sec80DB == '3' ) && age < 80)
		{
		j.setFieldError('scheduleVIA.usrDeductUndChapVIA.section80DHealthInsPremium.medicalExpenditure','Please select a valid option from the dropdown provided at Part (B) of Sec.80D under Chapter VI-A');
		addErrorXHTML('','Please select a valid option from the dropdown provided at Part (B) of Sec.80D under Chapter VI-A',true);
		}
...
function populateFullCon(a,b,c) {
	...
	if(parseInt(coalesce(afield.value),10)==0 &&  parseInt(coalesce(bfield.value),10)==0){
...
function calcTotFullConTable() {
	...
	for (var i = 0; i < (noOfRows / 16); i++) {
		...
		if (parseInt(
				coalesce(document
						.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['
								+ i + '].fullValueConsdRecvUnqshr')[0].value), 10) > parseInt(
				coalesce(document
						.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['
								+ i + '].fairMrktValueUnqshr')[0].value), 10)) {

			document
					.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['
							+ i + '].fullValueConsdSec50CA')[0].value = parseInt(
					coalesce(document
							.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['
									+ i + '].fullValueConsdRecvUnqshr')[0].value), 10);
...
function calcTotFullCon()
{
	document.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullConsideration')[0].value
	= eval(coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdSec50CA') +
			coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.nRISecur115AD.fullValueConsdOthUnqshr'));

}


function calcTotFullSale()
{
	document.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.saleOnOtherAssets.fullConsideration')[0].value
	= eval(coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.saleOnOtherAssets.fullValueConsdSec50CA') +
			  coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.saleOnOtherAssets.fullValueConsdOthUnqshr'));


}

function calcTotFullB1B6()
{
	document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.saleofAssetNA.deductSec48.fullConsideration')[0].value
	= eval(coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.saleofAssetNA.fullValueConsdSec50CA') +
			  coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.saleofAssetNA.fullValueConsdOthUnqshr'));

}

function populateTotFullfor5LTCG()
{
	var tab = document.getElementById('stcg10pctTab');
	var noOfRows = tab.rows.length;

	for (var i = 0; i < (noOfRows - 15); i++) {
	var  fullValueConsdSec50CA =  parseInt(document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdSec50CA')[0].value);
	var  fullValueConsdRecvUnqshr =  parseInt(document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdRecvUnqshr')[0].value);
	var  fairMrktValueUnqshr =  parseInt(document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fairMrktValueUnqshr')[0].value);

	if (fullValueConsdRecvUnqshr > fairMrktValueUnqshr) {
		document
				.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdSec50CA')[0].value = document
				.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdRecvUnqshr')[0].value;
	}
	if (fullValueConsdRecvUnqshr < fairMrktValueUnqshr)
	{
		document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdSec50CA')[0].value = document
		.getElementsByName('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fairMrktValueUnqshr')[0].value;
	}
	}
}

function calcTotFullfor5LTCG()
{
	var tab = document.getElementById('stcg10pctTab');
	var noOfRows = tab.rows.length;
	...
	for (var i = 0; i < (noOfRows/16); i++) {
		...
		= eval(coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdSec50CA') +
				  coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.nRIOnSec112and115['+ i +'].fullValueConsdOthUnqshr'));
	}
...

{
	...
	for (var i = 0; i < (noOfRows5 - 2); i++) {
		amt5 = document
				.getElementsByName('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].amtDeducted')[0].value;
	...
	document.getElementsByName('scheduleCGPost45.deducClaimInfo.totDeductClaim')[0].value = eval(parseInt(sum, 10) + parseInt(sum1, 10) +
			parseInt(sum2, 10) + parseInt(sum3, 10) + parseInt(sum4, 10) + parseInt(sum5, 10) + parseInt(sum6, 10));
...

{
	...
	for (var i = 0; i < (noOfRows - 2); i++) {
		...
		if (rateAsPerTreaty > rateAsPerITAct) {
			document
					.getElementsByName('scheduleOS.incChargblSplRateOS.nriIncTaxDtaa['
							+ i + '].applicableRate')[0].value = rateAsPerITAct;
		}
		...
		if (rateAsPerITAct > rateAsPerTreaty)
		{
			document
					.getElementsByName('scheduleOS.incChargblSplRateOS.nriIncTaxDtaa['
							+ i + '].applicableRate')[0].value = rateAsPerTreaty;
		}
		if (rateAsPerTreaty == rateAsPerITAct) {
			document
					.getElementsByName('scheduleOS.incChargblSplRateOS.nriIncTaxDtaa['
...
function validateSchCGSTCGAmt() {
	...
	var totalStcg = eval(coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.stcgUnderDtaa.totAmtStcgUnderDtaa')
			+ coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.stcgUnderDtaa.totalAmtNotTaxUsDTAAStcg'));
	...
	{
		j.setFieldError('',
				'The amount of A7a +A7b should be equal to amount in STCG DTAA table');
		addErrorXHTML('',
				'The amount of A7a +A7b should be equal to amount in STCG DTAA table', true);
...
function validateSchCGLTCGAmt() {
	...
	var totalStcg = eval(coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.ltcgUnderDtaa.totAmtLtcgUnderDtaa')
			+ coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.ltcgUnderDtaa.totalAmtNotTaxUsDTAALtcg'));
	...
	{
		j.setFieldError('',
				'The amount of B9a +B9b should be equal to amount in LTCG DTAA table');
		addErrorXHTML('',
				'The amount of B9a +B9b should be equal to amount in LTCG DTAA table', true);
...
function validateTDS29Grtthen67() {
			...
			if (amt9.value > tempVal) {
				j
						.setFieldError('scheduleTDS2.tdSonOthThanSal[' + i
								...
								'Amount cannot be more than the amount entered in Col (6) or col (7)');
				addErrorXHTML(
						document
	...
	if (amt9.value > tempVal) {
		j
				.setFieldError('scheduleTDS2.tdSonOthThanSal[' + i
						+ '].taxDeductCreditDtls.taxCreditedOwnHands',
						'Amount in field Col (9) cannot be more than Col (6) + TDS field of col (8)');
		addErrorXHTML(
				document
						.getElementsByName('scheduleTDS2.tdSonOthThanSal['
								+ i
...
function calcTotalOtherAllow() {
	...
	try {
		...
		for (var i = 0; i < salaryCount; i++) {
			...
			document.getElementsByName('scheduleS.salaries[' + i
					+ '].salarys.otherAllowancesTot')[0].value = sum;
...
function populateSectionCode(elem,subCatVal){
	...
	if(index==0){
		...
		subCat.options[2]=new Option('54F','54F');
		subCat.options[3]=new Option('54GB','54GB');
	}
	else if(index==1){
...
function getTotForSalaryOtherAllowances(){
	...
	for (var i = 0; i < rowCount; i++) {
		...
		otherAllowancesSalaryTot = otherAllowancesSalaryTot
				+ eval(parseInt(coalesce(document.getElementsByName('scheduleS.salaries['+i+'].salarys.otherAllowancesTot')[0].value), 10));

	}
	return otherAllowancesSalaryTot;
}

function populateCGB8SecCode(){
		...
		for (var m= 0; m < subTable.rows.length - 3; m++) {
			...
			if (natureDesc == 'OTH' && othNatOfInc == '') {
				...
				if (natureDesc == 'OTH' && othNatOfInc != '' && othNatOfAmt == '') {
					...
					j.setFieldError('scheduleS.salaries[' + i + '].salarys.otherAllowances.othersIncDtls[' + m + '].othAmount',
							'Please enter the Amount.');
...
function validateDedUs54B()
{
	...
	if(date != '')
		...
		if(dateofPurchase != '' && cost == '')
				...
				addErrorXHTML(
						'',
						'Please enter the Cost of new agricultural land.');
...
{
	var tab = document.getElementById('schduleCGDedD1E');
	var noOfRows = tab.rows.length;
...
function validateDedUs54GB()
{
	...
	var	date = document
		.getElementsByName('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].dateofTransfer')[0].value;
	
	var	cost = document
		.getElementsByName('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].costofNewPlantMach')[0].value;
	
	var	dateofPurchase = document
	.getElementsByName('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].dateofPurchase')[0].value;
	
	var	amtDeposited = document
	.getElementsByName('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].amtDeposited')[0].value;
	...
	if(date != '')
		...
		j.setFieldError('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].dateofPurchase',
				'Please enter the date of purchase.');
	...
	return;
		...
		if(dateofPurchase != '' && cost == '')
			...
			addErrorXHTML(
					'',
					'Please enter the Cost of new plant and machinery.');

			j.setFieldError('scheduleCGPost45.deducClaimInfo.deducClaimDtlsUs54GB[' + i + '].costofNewPlantMach',
					'Please enter the Cost of new plant and machinery.');
		...
		if(dateofPurchase == '' && cost == '' && amtDeposited == '')
			...
			addErrorXHTML(
					'',
					'Please fill either Cost of new plant and machinery purchased & Date of purchase or Amount deposited in Deduction claimed u/s 54GB');
...
function editableCFLFields()
{
	var	returnFileSec = document.getElementsByName('partAGEN1.filingStatus.returnFileSec')[0].value;	
	
	if (returnFileSec == '12' || returnFileSec == '13' || returnFileSec == '20') {
