...
function calcBulidingSellSTCG(){
   ...
   var exemptionOrDednUs54 = coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.saleofLandBuild.exemptionOrDednUs54');
...
function calcOtherAssetSellSTCG(){
    ...
    capgainonAssets.value = eval(parseInt(balanceCG.value, 10) +
                     coalesceSetRet('scheduleCGPost45.shortTermCapGainPost45.saleOnOtherAssets.lossSec94of7Or94of8'));     
}
...
function calcDtaaTaxLTCG(){
	...
	/*var totAmtStcgUnderDtaa = document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.ltcgUnderDtaa.totAmtLtcgUnderDtaa')[0];
	   
	totAmtStcgUnderDtaa.value = addCGDeductions('scheduleLtcgDtaa');*/
...
function calculateLTCG(cgosIncome){
    try{
    	...
    	cgosIncome.cgInc.ltcg.prctg10.secProviso = parseInt(secSell) - setOffWithDtaa('secProviso');
    	...
    	cgosIncome.cgInc.ltcg.prctg10.sec115E_b = coalesceSetRet('scheduleCGPost45.longTermCapGainPost45.nRISaleofForeignAsset.balonSpeciAsset')  - setOffWithDtaa('sec115E_b');
        	...
        	alert(' exception in calculateLTCG :: '+e)
        }
}

function setOffWithDtaa(sectionCode) {
	
	var arr = {'B1e':0,'B2e':0,'B3e_22':0,'B3e_5ACA1b':0,'B4c':0,'B5e_21ciii':0,'B5e_5AC1c':0,'B5e_5ADiii':0,'B6c':0,'B6f':0,'B7e':0,'B8':0};
	var tab = document.getElementById('scheduleLtcgDtaa');
	  ...
	  for(var i=0; i<rowCount; i++) {
	   ...
	   if(itemIncluded == 'B1e') {
	       arr['B1e'] = parseInt(arr['B1e'],10) +  parseInt(coalesce(amount),10);
		       ...
		       arr['B3e_22'] = parseInt(arr['B3e_22'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B3e_5ACA1b') {
		       arr['B3e_5ACA1b'] = parseInt(arr['B3e_5ACA1b'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B4c') {
		       arr['B4c'] = parseInt(arr['B4c'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B5e_21ciii') {
		       arr['B5e_21ciii'] = parseInt(arr['B5e_21ciii'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B5e_5AC1c') {
			   arr['B5e_5AC1c'] = parseInt(arr['B5e_5AC1c'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B5e_5ADiii') {
		       arr['B5e_5ADiii'] = parseInt(arr['B5e_5ADiii'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B6c') {
		       arr['B6c'] = parseInt(arr['B6c'],10) +  parseInt(coalesce(amount),10);
		} else if(itemIncluded == 'B6f') {
		       arr['B6f'] = parseInt(arr['B6f'],10) +  parseInt(coalesce(amount),10);
	  ...
	  if(sectionCode == 'sec112') {
		  total = arr['B1e'] + arr['B2e'] + arr['B7e'] + arr['B4c'] + arr['B8'];
...
//To setOfff STCG With A7
function setOffSTCGWithA7(cgosIncome) {
	...
	for(var i=0; i<rowCount; i++){
		...
		if(sectionValue == 'A2e_111A' || sectionValue == 'A3a'){
			sec111a -= amount;
		} else if(sectionValue == 'A2e_115AD'){
			sec115ad_1_b_ii -= amount;
		} else if(sectionValue == 'A4e'){
			cgosIncome.cgInc.stcg.prctg30 -= amount;
		} else if(sectionValue == 'A1e' || sectionValue == 'A3b' || sectionValue == 'A5e' || sectionValue == 'A6'){
...
//To setOff LTCG With B9
function setOffLTCGWithB9(cgosIncome) {
...
function getSlabbedIncome(totalIncome){
	...
	if(taxPayer.value=='I' && age > 59 && age < 80 && (resStatus.value=='RES' || resStatus.value=='NOR')){
		...
		if(parseInt(netTxblIncome,10) >= parseInt('0',10) && parseInt(netTxblIncome,10) <= parseInt('300000',10)){
			...
			inc = {"5":(parseInt(netTxblIncome,10) - 300000), "20":0, "30":0};
		}else if(parseInt(netTxblIncome,10) >= parseInt('500001',10) && parseInt(netTxblIncome,10) <= parseInt('1000000',10)){
			inc = {"5":200000, "20":(parseInt(netTxblIncome,10) - 500000), "30":0};
		}else if(parseInt(netTxblIncome,10) >= parseInt('1000001',10)){
			inc = {"5":200000, "20":500000, "30":(parseInt(netTxblIncome,10) - 1000000)};
		}
		...
		if(parseInt(netTxblIncome,10) >= parseInt('0',10) && parseInt(netTxblIncome,10) <= parseInt('500000',10)){
			...
			inc = {"5":0, "20":(parseInt(netTxblIncome,10) - 500000), "30":0};
					
		}else if(parseInt(netTxblIncome,10) >= parseInt('1000001',10)){
			inc = {"5":0, "20":500000, "30":(parseInt(netTxblIncome,10) - 1000000)}; 
		}
		...
		if(parseInt(netTxblIncome,10) >= parseInt('0',10) && parseInt(netTxblIncome,10) <= parseInt('250000',10)){
			...
			inc = {"5":(parseInt(netTxblIncome,10) - 250000), "20":0, "30":0};
		}else if(parseInt(netTxblIncome,10) >= parseInt('500001',10) && parseInt(netTxblIncome,10) <= parseInt('1000000',10)){
			inc = {"5":250000, "20":(parseInt(netTxblIncome,10) - 500000), "30":0};
		}else if(parseInt(netTxblIncome,10) >= parseInt('1000001',10)){
			inc = {"5":250000, "20":500000, "30":(parseInt(netTxblIncome,10) - 1000000)};
		}
