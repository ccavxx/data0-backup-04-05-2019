...
function getDueDate234A(){
		var duedate = '31/07/2018'; //// Modified due to the due date extended for filing the return
...
try{
        ...
        if((filingType=='17'|| filingType=='18') && originalFilingDate !=undefined && originalFilingDate!=null && originalFilingDate!=''){
  		...
  		if (filingType=='11' || filingType=='12' || filingType=='17' ||filingType=='18' || filingType=='13' ){
  			...
  			var grossInc = document.getElementsByName('partBTI.grossTotalIncome')[0].value;
  			var gtiInc =grossInc+coalesce(document.getElementsByName('scheduleEI.ltcgWhereSTTPaid')[0].value);
  			var currentDate234F = '01/08/2018';
...
function validateTDS2(){
	...
	for (var i = 0; i < mainTabRows - 6; i++) {
		...
		if (taxDeductedOwnHands !='' && totTDSOnAmtPaid !=''){
			...
			addErrorXHTML(
					'',
					'Unclaimed TDS brought forward and details of TDS of current FY should be provided in different rows in 15B');
			j.setFieldError('scheduleTDS2.tdSonOthThanSal[' + i+ '].totTDSOnAmtPaid','Unclaimed TDS brought forward and details of TDS of current FY should be provided in different rows in 15');
