...
//Part C limited to gtiLimitC
	...
	if(eval(parseInt(sec80P.value,10)>parseInt(gtiLimitC,10))){
		...
		sec80PSysCal.value =parseInt(coalesce(sec80P.value),10);
	...
	sumDeductions(sec80GSysCal,sec80GGCSysCal,
			sec80IASysCal,sec80IABSysCal,sec80IACSysCal,sec80IBSysCal,sec80IBASysCal,sec80ICSysCal,sec80IDSysCal,sec80JJASysCal,
...
function sumUserEntrdDed() {
	...
	dedPartC.value= eval(sec80IASysCal.value)+ eval(sec80IABSysCal.value)+eval(sec80IACSysCal.value)+eval(sec80ICSysCal.value)+ eval(sec80IDSysCal.value)+ eval(sec80IBSysCal.value)+
	...
	function calScheduleAmtc(){
		...
		// amtCreditBalBroughtFwd4.value = parseInt(coalesce(amtCreditFwd4),10); 
		 amtCreditBalBroughtFwd5.value = parseInt(coalesce(amtCreditFwd5),10); 
		 
		 totAMTGross = parseInt(coalesce(amtCreditFwd),10) + parseInt(coalesce(amtCreditFwd1),10) + parseInt(coalesce(amtCreditFwd2),10)+parseInt(coalesce(amtCreditFwd3),10)+parseInt(coalesce(amtCreditFwd4),10)+parseInt(coalesce(amtCreditFwd5),10); 
		...
		balAmtCreditCarryFwd5.value= parseInt(coalesce(amtCreditBalBroughtFwd5.value),10) - parseInt(coalesce(amtCreditUtilized5),10);
		 ...
		 totAmtCreditUtilisedCY = parseInt(coalesce(amtCreditUtilized),10) + parseInt(coalesce(amtCreditUtilized1),10)+ parseInt(coalesce(amtCreditUtilized3),10)+parseInt(coalesce(amtCreditUtilized4),10)+parseInt(coalesce(amtCreditUtilized5),10); 
		 totBalAMTCreditCF = parseInt(coalesce(balAmtCreditCarryFwd.value),10) + parseInt(coalesce(balAmtCreditCarryFwd1.value),10) + parseInt(coalesce(balAmtCreditCarryFwd2.value),10)+ parseInt(coalesce(balAmtCreditCarryFwd3.value),10)+parseInt(coalesce(balAmtCreditCarryFwd4.value),10)+parseInt(coalesce(balAmtCreditCarryFwd5.value),10); 
		 
		
		 document.getElementsByName('scheduleAMTC.totSetOffEys')[0].value= parseInt(coalesce(amtCreditSetOfEy1),10) + parseInt(coalesce(amtCreditSetOfEy),10)+parseInt(coalesce(amtCreditSetOfEy3),10)+parseInt(coalesce(amtCreditSetOfEy4),10);
...
function populateAMT(){
	...
	eval(parseInt(coalesce(sec80ICSysCal.value),10))+eval(parseInt(coalesce(sec80IDSysCal.value),10))+eval(parseInt(coalesce(sec80JJASysCal.value),10))+eval(parseInt(coalesce(sec80IBASysCal.value),10))+
	...
	if(subuserType.value=="7" || subuserType.value=="8" || userType.value=="9"|| subuserType.value=="3" || subuserType.value=="4" ){
...
try
{
	...
	for(var i = 0; i < tab.rows.length-3; i++) {
        ...
        balancePLOthThanSpecBus.value = eval(parseInt(profBfrTaxPL.value ,10)) - eval(parseInt(netPLFromSpecBus.value ,10)) - eval(parseInt(netProfLossSpecifiedBus.value,10)) - eval(parseInt(incRecCredPLOthHeadsHp.value,10)) - eval(parseInt(incRecCredPLOthHeadsCg.value,10)) - eval(parseInt(incRecCredPLOthHeadsOs.value,10)) - eval(parseInt(incRecCredPL5BBFHeadsOs.value,10)) - eval(parseInt(plUs44SChapXIIG.value,10)) - eval(parseInt(totExempInc.value,10));
        // 9. Total (7 + 8)
        totExpDebPL.value = eval(parseInt(expDebToPLOthHeadsHouseProperty.value,10)) + eval(parseInt(expDebToPLOthHeadsCapitalGains.value,10)) + eval(parseInt(expDebToPLOthHeadsOthSources.value,10))  +eval(parseInt(expDebToPLOthHeadsUndSec115BBF.value,10))  + eval(parseInt(expDebToPLExemptInc.value,10)) ;
...
function calculateTaxPayableOnTotalInc(netTxblIncome, addExemption){
	...
	if(taxPayer.value=='9' || (taxPayer.value=='14' && subtaxPayer.value=='11') ){
		...
		}/*else if (firstFlag=='Y' && secondFlag=='N' ){

		    main.generateMsgDialogWithOk('Under this scenario, filing of ITR 5 under the status PDT is not required. The share of income should be reflected in the personal return of income',"");
		}*/
	...
	}else if(taxPayer.value=='14' && (subtaxPayer.value=='3' || subtaxPayer.value=='4')){
...
function getDueDate234A(){
		var duedate = '05/08/2017'; //// Modified due to the due date extended for filing the return
...
try{
		...
		if(getDueDate234A() == '05/08/2017'){
			
			duedate = '06/08/2017';
		}else if(getDueDate234A() == '07/11/2017'){
			
			duedate = '08/11/2017';
		}else if(getDueDate234A() == '30/11/2017'){
			...
			if(parseInt(advanceTax,10) < ((parseInt(balTaxPayable,10) - parseInt(TDS,10) - parseInt(TCS ,10)) * parseFloat('0.90'))) {
				...
				for(var p = 0; p < allInputTags.length; p++) {
					if(allInputTags[p].name.match("dateDep$")){
						if( checkFirstDateBefore('01/04/2017' , allInputTags[p].value) && checkFirstDateBefore(allInputTags[p].value, currentDate) ){
				...
				if(selfAsspaidDates.length==0){
					noOfMonthsTillSelfasst=calcNoOfMonths(currentDate,'01/04/2017');
				}else{
					noOfMonthsTillSelfasst=calcNoOfMonths(selfAsspaidDates[0],'01/04/2017');
				}
...
function calculate234cIntrst(TDS, TCS, slabs, cgosIncome,AMT){
	...
	var normalBalIncome = eval(
						parseInt(coalesceSetRet('partBTI.grossTotalIncome')) 
						- totalCG- parseInt(coalesceSetRet('partBTI.incFromOS.incChargeSplRate')) - parseInt(coalesceSetRet('itr5ScheduleBP.businessIncOthThanSpec.incRecCredPLOthHeads.underSec115BBF'))
						- parseInt(coalesceSetRet('partBTI.totalDeductionsUnderScheduleVIA'))
                                                    - parseInt(coalesceSetRet('partBTI.deductionsUnder10Aor10AA'))
						);	
	...
	if(parseInt(coalesceSetRet('partBTTI.computationOfTaxLiability.taxPayableOnDeemedTI.totalTax'))!=
		...
		if(parseInt(coalesceSetRet('partBTTI.computationOfTaxLiability.taxPayableOnTI.surchargeOn2F'))>0){
			...
			//SISurcharge= (parseInt(SurchargeUnderSec)*(siTax/TaxpayableUnderSec));
			...
			if( statusCompany == "1" || statusCompany == "2" || statusCompany == "3" || statusCompany == "4" || statusCompany == "5"){
	...
	var oldTotalUp16Of12To15Of3 = totalUp16Of12To15Of3;
	 ...
	 if(marginal1 || (secIncome > 0 && ((coalesce(parseInt(document.getElementsByName('scheduleVIA.deductUndChapVIA.totalDeductionPartC')[0].value, 10))+
		 ...
		 //totalUp16Of12To15Of3 = parseInt(totalUp16Of9To15Of12) + (parseInt(actualTax[3]) +parseInt(coalesce( parseInt(SurchargeUnderSec) * coalesce(actualTax[3]/TaxpayableUnderSec)))) * 1.03;
		// totalUp16Of3To31Of3 = (parseInt(actualTax[4]) + parseInt(coalesce( parseInt(SurchargeUnderSec) * coalesce(actualTax[4]/TaxpayableUnderSec)))) * 1.03;
...
function calsurchargeOnAmtcAboveCrore(){
	...
	// for surcharge
				...
				if( rndOffNrsTen(totInc.value) > 10000000 ){
					...
					if( statusCompany == "1" || statusCompany == "2" || substatusCompany == "3" || substatusCompany == "4" || substatusCompany == "5"){
	...
	for(var i = 0; i < allInputTags.length; i++) {
		if(allInputTags[i].name.match("dateDep$")){
			if(checkFirstDateBefore('01/04/2016' , allInputTags[i].value) && checkFirstDateBefore(allInputTags[i].value , '31/03/2017')){
				...
				}else if( checkFirstDateBefore('01/04/2017' , allInputTags[i].value)){
...
function calculateAdvancedTax234A(date){
	...
	for(var i = 0; i < allInputTags.length; i++) {
		if(allInputTags[i].name.match("dateDep$")){
			if(checkFirstDateBefore('01/04/2017' , allInputTags[i].value) && checkFirstDateBefore(allInputTags[i].value , date)){
					advanceTax = eval( parseInt(isNVL(advanceTax) ,10) + parseInt(isNVL(allInputTags[i+2].value) ,10));
				}
		}
	}
...
function calculateSchDOA() {
    ...
    // For Field 10
     ...
     $('[name="scheduleDOA.building.rate100.depreciationDetail.totalDepreciation"]')[0].value = zeroOrMore(coalesceSetRet("scheduleDOA.building.rate100.depreciationDetail.depreciationAtFullRate") + coalesceSetRet("scheduleDOA.building.rate100.depreciationDetail.depreciationAtHalfRate"));
...
function adjustFormPartAGEN2(){
	...
	if(substatusCompany==8 || substatusCompany==12 && (fourth!='J'||fourth!='G')){
    ...
    if(substatusCompany== '3' || substatusCompany== '4'){
...
function setCG54Dtls() {
	...
	for ( var i = 0; i < (noOfRows-3); i++) {
			...
			if(section != '') {
				document.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.unUtilizedCapGain54['+i+'].ay')[0].value = '2013-14';
			} else {
	...
	for ( var i = 0; i < (noOfRows-3); i++) {
			...
			if(section != '') {
				document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.unUtilizedCapGain54['+i+'].ay')[0].value = '2013-14';
			} else {
...
function calcSchCGLtcgStcg(){
	...
	//dropDownSelection('ltcgDeduction54ec54ee3');
	dropDownSelection('ltcgDeduction54ec54ee4');
//	dropDownSelection('stcg10pctTab_ded1');
...
function calculateOSGross(simap){
	try {
		...
		for ( var i = 0; i < allSelects.length; i++) {
			...
			if (allSelects[i].value == 'Others') {
				...
				if (allSelects[i].value == '5BBDA') {
					var divInc5BBDA = document.getElementsByName('scheduleOS.incChargblSplRateOS.dividendIncome115BBDA')[0];
					divInc5BBDA.value = document.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['+ index + '].sourceAmount')[0].value;
				} 
				
				if (allSelects[i].value == '5BBE') {
					var divInc5BBE = document.getElementsByName('scheduleOS.incChargblSplRateOS.dividendIncome115BBE')[0];
					divInc5BBE.value = document.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['+ index + '].sourceAmount')[0].value;
				} 
				
				if (allSelects[i].value == '5BBF') {
					var divInc5BBF = document.getElementsByName('scheduleOS.incChargblSplRateOS.dividendIncome115BBF')[0];
					divInc5BBF.value = document.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['+ index + '].sourceAmount')[0].value;
				} 
		...
		secXIIOth.value = zeroOrMore(eval(sum - temp - temp115BB - temp115BBDA - temp115BBF - temp115BBE));
...
function adjustForm(){
	try{
			...
			enableField('partAGEN1.orgFirmInfo.subStatus',['3','4','5','7','10'],
					'partapl.debitsToPL.debitPlAcnt.salRemuneration'
					);
			...
			enableField('partAGEN1.orgFirmInfo.subStatus',['3'],
					'itr5ScheduleBP.businessIncOthThanSpec.deemedProfitBusUs.section44DB'
					);
			/*enableField('partAGEN1.orgFirmInfo.statusOrCompanyType',['1'],
					'scheduleVIA.usrDeductUndChapVIA.section80GGC'
					);*/
			enableField('partAGEN1.orgFirmInfo.subStatus',['3','4','5','7','8','10','11','12','13'],
					'scheduleVIA.usrDeductUndChapVIA.section80GGC'
					);
...
function calcScheduleSI(){
	try{
			...
			//5BBF BP
		    /*var secCodeBBF =  document.getElementsByName('scheduleSI.splCodeRateTax[9].secCode')[0]; secCodeBBF.value = coalesceString(secCodeBBF.value);
			var splRatePercentBBF = document.getElementsByName('scheduleSI.splCodeRateTax[9].splRatePercent')[0]; splRatePercentBBF.value = coalesce(splRatePercentBBF.value);
			var splRateIncBBF = document.getElementsByName('scheduleSI.splCodeRateTax[9].splRateInc')[0]; splRateIncBBF.value = coalesce(splRateIncBBF.value);
			var taxableIncBBF = document.getElementsByName('scheduleSI.splCodeRateTax[9].taxableInc')[0]; taxableIncBBF.value = coalesce(taxableIncBBF.value);
			var splRateIncTaxBBF = document.getElementsByName('scheduleSI.splCodeRateTax[9].splRateIncTax')[0]; splRateIncTaxBBF.value = coalesce(splRateIncTaxBBF.value);
			*/
			...
			//taxableIncBBF.value=zeroOrMore(splRateIncBBF.value);
	   ...
	   deleteSIRow();
	    ...
	    for(var i in simap){
	        if(simap[i]!='undefined' && simap[i]!=null && simap[i]!=0){
	            ...
	            if(!secAdded && !(i=='5BBE68' || i=='5BBE69' ||i=='5BBE69A' || i=='5BBE69B' ||i=='5BBE69C' || i=='5BBE69D')){
...
function validatePartners(){
	...
	if(substatus == 5 || status == 1|| substatus == 3|| substatus == 4 || substatus == 8 || substatus == 11){
...
function validateCGSecwiseDed(){
	var arr = {'54':0,'54B':0,'54EC':0,'54GB':0,'54D':0,'54G':0,'54GA':0,'54EE':0};
	var arrDed = {'54':0,'54B':0,'54EC':0,'54GB':0,'54D':0,'54G':0,'54GA':0,'54EE':0};
	...
	//sumSectionWise('ltcgDeduction54ec54ee3', arr);
...
function setOtherGrossSource() {
	...
	var allSectionArray = [ '5A1ai', '5A1aii', '5A1aiia', '5A1aiiaa',
			'5A1aiiab', '5A1aiiac', '5A1aiii', '5A1bA', '5A1bB', '5AC1ab',
			 '5AD1i', '5AD1iP', '5BBA','5BBDA', '5BBC', '5BBF',  '1','5AB1a',
			'Others'];
...
function calculateRemurationPaid(){
	...
	var substatus = document.getElementsByName('partAGEN1.orgFirmInfo.subStatus')[0].value;
	var sum = 0;
	if(substatus == '8' || status == '2' || substatus == '3' || substatus == '4' || substatus == '7' || status == '9' ||substatus == '13' ||substatus == '11'|| substatus == '12') {
...
function verificationPanValidation(tableId){
	...
	var table = document.getElementById(tableId);
	var allInputTags = table.getElementsByTagName('input');
	if(status=='1'||substatus=='3'||substatus=='4'||substatus=='5'||substatus=='7'||substatus=='8'){
		for(var i = 0; i < allInputTags.length; i++) {
...
function subStatus()
{
	...
	if(statusCompany1 == '1'){
		...
		sub.options[0] = new Option('Select','-1');
		sub.options[1] = new Option('Cooperative Bank','3');
		sub.options[2] = new Option('Society Registered under Societies Registration Act-1860 or any law corresponding to that Act','11');
		sub.options[3] = new Option('Cooperative Society','4');
		sub.options[4] = new Option('Private Discretionary Trust','7');
		sub.options[5] = new Option('Estate of the deceased','12');
		sub.options[6] = new Option('Any other Trust/Institution','13');
		sub.options[7] = new Option('Any other AOP/BOI','8');
...
function priBankMandtry()
{
	...
	var cashDeposit = document.getElementsByName('itr.refund.cashDeposited')[0].value;
...
function enableBankDtls()
{
	...
	enableField('itr.scheduleBA.bankDtlsFlag','Y','itr.refund.cashDeposited');
...
function enableGen2ForPDTAOT(){
	...
	if(substatusCompany==7 || substatusCompany==13 && (fourth!='J'||fourth!='G')){
