...
function getIfscBankDetails(elem){
	...
	var  ifscCode = document.getElementsByName('itr1.refund.scheduleBA['+position+'].ifscCode')[0].value;
	
	document.getElementsByName('itr1.refund.scheduleBA['+position+'].bankName')[0].value = main.getBankName(ifscCode);
...
function getIfscBankName(field){
	...
	document.getElementsByName('itr1.refund.scheduleBA.priBankDetails.bankName')[0].value = main.getBankName(field.value);
...
/*	if(parseInt(totInc.value)>5000000){
		alert('insidee'+totInc.value);
		addErrorXHTML('' ,'Taxable Total Income at C2 can not exceed 50 Lakhs.');	
		j.setFieldError('itr1.itr1IncomeDeductions.totalIncome','Taxable Total Income at C2 can not exceed 50 Lakhs.');
		return;
...
function sumDeductions(sec80CsysCalc,sec80CCCsysCalc,sec80CCDemprContrsysCalc,sec80CCDempeContrsysCalc,
		sec80CCD1BsysCalc,sec80DsysCalc,sec80DDsysCalc,sec80DDBsysCalc,sec80EsysCalc,sec80EEsysCalc,sec80GsysCalc,
		sec80GGsysCalc,sec80GGAsysCalc,sec80GGCsysCalc,sec80UsysCalc,sec80CCGsysCalc,
		sec80RRBsysCalc,sec80QQBsysCalc,sec80TTAsysCalc){
...
function calcEduCess(){
	...
	eduCess.value = eval(coalesce(taxPayableOnRebate))* eval('0.03');
...
function calcBalTaxPay(){
	...
	totTaxWithEduCess.value= eval(parseInt(eduCess.value,10)) + eval(parseInt(taxPayableOnRebate.value,10));
...
function calcInterestPayable(){
		...
		for(var i = 0; i < allInputTags.length; i++) {
			if (allInputTags[i].name.match("amtForTaxDeduct$") && allInputTags[i+1].name.match("totTDSOnAmtPaid$")) {
				if(eval(parseInt(coalesce(allInputTags[i].value),10) <  (2*parseInt(allInputTags[i+1].value,10)))){
					addError(allInputTags[i],'Amount which is subject to tax deduction could not be less than twice the amount of tax deducted',true);
					j.setFieldError(allInputTags[i].name,'Amount which is subject to tax deduction could not be less than twice the amount of tax deducted');
					//allInputTags[i].value = parseInt('0',10);
				}
			}
		}
		...
		for(var i = 0; i < allInputTags.length; i++) {
			if (allInputTags[i].name.match("amtTaxCollected$") && allInputTags[i+1].name.match("totalTCS$")) {
				if(eval(parseInt(coalesce(allInputTags[i].value),10) <  (2*parseInt(allInputTags[i+1].value,10)))){
					addError(allInputTags[i],'Amount which is subject to tax collection could not be less than twice the amount of tax collected',true);
					j.setFieldError(allInputTags[i].name,'Amount which is subject to tax collection could not be less than twice the amount of tax collected');
					//allInputTags[i].value = parseInt('0',10);
				}
			}
		}
		...
		for(var i = 0; i < allInputTags.length; i++) {
			if (allInputTags[i].name.match("dateDep$")) {
				...
				if(checkFirstDateBefore('01/04/2016' , allInputTags[i].value) && checkFirstDateBefore( allInputTags[i].value , '15/06/2016' ) ){						
					slab1 = eval(parseInt(slab1 ,10) + parseInt(isNVL(allInputTags[i+2].value) ,10));
				}
				else if(checkFirstDateBefore('16/06/2016' , allInputTags[i].value) && checkFirstDateBefore( allInputTags[i].value , '15/09/2016' ) ){						
					slab2 = eval(parseInt(slab2 ,10) + parseInt(isNVL(allInputTags[i+2].value) ,10));
				}
				else if(checkFirstDateBefore('16/09/2016' , allInputTags[i].value) && checkFirstDateBefore( allInputTags[i].value , '15/12/2016' ) ){						
					slab3 = eval(parseInt(slab3 ,10) + parseInt(isNVL(allInputTags[i+2].value) ,10));
				}
				else if(checkFirstDateBefore('16/12/2016' , allInputTags[i].value) && checkFirstDateBefore( allInputTags[i].value , '15/03/2017') ){
...
document.getElementsByName('itr1.refund.scheduleBA.priBankDetails.ifscCode')[0].value=document.getElementsByName('itr1.refund.scheduleBA.priBankDetails.ifscCode')[0].value.toUpperCase();
...
function checkBankAcntScheduleBA() {
	try {
		...
		for ( var i = 0; i < eval(parseInt(noOfRows, 10) - 2); i++) {
			var bankaccNo = document.getElementsByName('itr1.refund.scheduleBA[' + i
					+ '].bankAccountNo')[0].value;
			if (!(pat.test(bankaccNo))) {
				j.setFieldError('itr1.refund.scheduleBA[' + i + '].bankAccountNo',
