...
function dedUndVIA(){
	...
	var sec80EEsysCalc = document.getElementsByName('itr4S.itr4SIncomeDeductions.deductUndChapVIA.section80EE')[0];
	...
	if( parseInt(sec80C,10) > eval('150000')) {
		...
		if( parseInt(sec80C,10) > parseInt(grossTotInc)) {
	...
	if( parseInt(sec80D,10) > eval('20000')){
		
		sec80DsysCalc.value = '20000';
		} else {
			sec80DsysCalc.value = parseInt(sec80D,10);
		}
		if(parseInt(grossTotInc ,10) < sec80DsysCalc.value){
	...
	if( parseInt(grossTotInc ,10) >0 ){
		if(residentialStatus =='RES' || residentialStatus =='NOR'){
			if( parseInt(sec80DD,10) > eval('100000')){
	...
	if( parseInt(grossTotInc ,10) >0 ){
		if(residentialStatus ==='RES' || residentialStatus ==='NOR'){
			if(status =='I'){
				if( parseInt(sec80U,10) > eval('100000')){
	...
	sec80GGsysCalc.value=0;
		...
		sec80UsysCalc,sec80CCGsysCalc,sec80RRBsysCalc,sec80QQBsysCalc,sec80TTAsysCalc,sec80EEsysCalc);
...
function sumDeductionsWithout80GG(sec80CsysCalc,sec80CCCsysCalc,sec80CCDemprContrsysCalc,sec80CCDempeContrsysCalc,
		sec80DsysCalc,sec80DDsysCalc,sec80DDBsysCalc,sec80EsysCalc,sec80GsysCalc,
		sec80GGCsysCalc,sec80UsysCalc,sec80CCGsysCalc,sec80RRBsysCalc,sec80QQBsysCalc,sec80TTAsysCalc,sec80EEsysCalc){
...
function returnFile_UnderSec(type){
	...
	if(status.value=='H'||status.value=='S'){
...
if((pan.substring(3,4)=='P' || pan.substring(3,4)=='p') && (status == 'H')){
		addErrorXHTML('' ,'Since PAN entered is Individual PAN, Select Status in A14 as Individual');	
		j.setFieldError('itr4S.personalInfo.pan','Since PAN entered is Individual PAN, Select Status in A15 as Individual');
		} else if((pan.substring(3,4)=='H' || pan.substring(3,4)=='h') && (status == 'I')){
	addErrorXHTML('' ,'Since PAN entered is HUF PAN, Select Status in A14 as HUF');	
	j.setFieldError('itr4S.personalInfo.pan','Since PAN entered is HUF PAN, Select Status in A15 as HUF');
	}
...
function checkNoOfRowsFilled(){
	...
	if(noOfBankAccounts != '' && (noOfBankAccounts-1) != count){
		 j.setFieldError('itr4S.scheduleBA.noOfBankAcc','Number of bank accounts held during the pervious year should be equal to number of rows entered in the below tables');
	}
