...
public class TaxDeductedFrmIncomeWS2_prev  implements Initializable{
	...
	/*	ITRView form = (ITRView) Form.getForm();
		List<TDSonOthThanSalB> list =(List<TDSonOthThanSalB>) ValidationUtil.ALL_TVC.get("class com.itd.efiling.offline.ITR4.ctrl.TaxDeductedFrmIncomePartB").getTableView().getItems();
		
		
		for(int i =0;i<list.size();i++)
		{
			TDSonOthThanSalB td = list.get(i);
		...
		if(OnChangeUtil.getElementValueByTargetName("itr.itr4.tdSonOthThanSals.totalTDSOthThanSalPartB").equals("empty"))
		{
			total.setText("0");
		}
		else
		{
			total.setText(OnChangeUtil.getElementValueByTargetName("itr.itr4.tdSonOthThanSals.totalTDSOthThanSalPartB"));
		}
		*/
