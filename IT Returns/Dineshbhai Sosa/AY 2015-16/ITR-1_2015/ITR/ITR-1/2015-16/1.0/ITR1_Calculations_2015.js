...
function calcInterestPayable(){
		...
		if(parseInt(balTaxPayable.value ,10) - parseInt(advanceTax ,10) - parseInt(TDS ,10) -parseInt(selfAssessmentTax234A,10) < 0){
			intrst234Aprinciple = parseInt('0' ,10);
		}else {
			intrst234Aprinciple = parseInt(balTaxPayable.value ,10) - parseInt(advanceTax ,10) - parseInt(TDS ,10)-parseInt(selfAssessmentTax234A,10);
...
function checkNoOfRowsFilled(){
	...
	if(noOfBankAccounts != '' && (noOfBankAccounts-1) != count){
		 j.setFieldError('itr1.scheduleBA.noOfBankAcc','Number of bank accounts held during the pervious year should be equal to number of rows entered in the below tables');
	}
