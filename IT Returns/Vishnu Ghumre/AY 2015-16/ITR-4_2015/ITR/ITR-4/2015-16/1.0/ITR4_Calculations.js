...
var FY_start_date = '01/04/2014';
var FY_end_date = '31/03/2015';
var AY_start_date = '01/04/2015';
var Filing_dueDate = '07/09/2015'; // Modified due to the due date extended for filing the return
var Int_start_date_234A = '08/09/2015'; // Modified due to the due date extended for filing the return

var slab1_end_date = '15/09/2014';
var slab2_start_date = '16/09/2014';
var slab2_end_date = '15/12/2014';
var slab3_start_date = '16/12/2014';
var slab3_end_date = '16/03/2015';
var slab4_start_date = '17/03/2015';
var Cg_ded_start_date = '31/03/2013';
var Cg_ded_end_date = '01/04/2018';
  ...
  try{
	...
	var selectsLength = document.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['+i+'].sourceDescription')[0].options.length;
		for (var k=selectsLength; k>0;  k--) {
      		   if(document.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['+i+'].sourceDescription')[0].options[k] != null && sectionArray.indexOf(document.getElementsByName('scheduleOS.incOthThanOwnRaceHorse.othersGrossDtls['+i+'].sourceDescription')[0].options[k].value) != -1) {
...
function setOtherGrossSource() {
	...
	var allSectionArray = [ '5A1ai', '5A1aii', '5A1aiia', '5A1aiiaa','5A1aiiab', '5A1aiiac', '5A1aiii', '5A1bA', '5A1bB', '5AC1ab','5ACA1a', '5AD1i', '5AD1iP', '5BBA', '5BBC', '5BBE', '5Ea', '1','Others' ];
...
// Function for calculating LTCG and STCG.
	function calcSchCGLtcgStcg(){
		...
		if(stcg30Per < shortTermUnder30Per) {
			j.setFieldError('scheduleCGPost45.accruOrRecOfCG.shortTermUnder30Per.upto15Of9', 'Total of Quarterly breakup in STCG 30% is more by ' + (stcg30Per-shortTermUnder30Per));
...
function calTotalEI(){
		...
		coalesceSetRet('scheduleEI.shareProfit')+
...
function addScheduleHP(){
	...
	if(flag ){
		...
		for(var j=0;j<cellsTot.length;j++){
			cloneNode.cells[j].innerHTML=cloneNode.cells[j].innerHTML.replace('1a',totRow+'a').replace('1b',totRow+'b').replace('1c',totRow+'c').replace('1d',totRow+'d').replace('1e',totRow+'e').replace('1f',totRow+'f').replace('1g',totRow+'g').replace('1h',totRow+'h').replace('1i',totRow+'i').replace('1j',totRow+'j').replace('property 1','property '+totRow).replace('property 1','property '+totRow);
		}
		...
		for(var a=0;a<selectTags.length;a++){
			...
			if(selectTags[a].name.indexOf('stateCode')!='-1'){
					selectTags[a].onchange = function(){populatePincode('scheduleHP.propertyDetails['+ iterate +'].addressDetail.stateCode','scheduleHP.propertyDetails['+ iterate +'].addressDetail.pinCode');};
				}
                        selectTags[a].id=selectTags[a].name.replace(/([\.\[\]])/g,'_').replace(/(__)/g,'_');
		...
		document.getElementById('scheduleHPMain').getElementsByTagName('tr')[0].parentNode.insertBefore(cloneNode,tobeInsertBefore);
		}
	...
	document.getElementsByName('scheduleHP.propertyDetails['+ (parseInt(totRow) - 1)  +'].hpSno')[0].value = totRow;
...
function scheduleHPCalcFor4(){
	...
	document.getElementsByName('scheduleHP.totalIncomeChargeableUnHP')[0].value = eval(parseInt(document.getElementsByName('scheduleHP.rentOfEarlierYrSec25AandAA')[0].value==""?0:document.getElementsByName('scheduleHP.rentOfEarlierYrSec25AandAA')[0].value) + parseInt(document.getElementsByName('scheduleHP.rentArearsSec25BAfter30PcDeduct')[0].value==""?0:document.getElementsByName('scheduleHP.rentArearsSec25BAfter30PcDeduct')[0].value) + parseInt(total));
}
...
function calculatePartBTTI_first(cgosIncome){
	...
	document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnDeemedTI.totalTax')[0].value = eval(
		...
		+ parseInt(coalesceSetRet('partBTTI.computationOfTaxLiability.taxPayableOnDeemedTI.educationCess'),10)
            ...
            calculateTaxPayableOnTotalInc('partBTI.aggregateIncome','partBTTI.computationOfTaxLiability.taxPayableOnTI.taxAtNormalRatesOnAggrInc');
		...
		if(coalesceSetRet('partBTI.aggregateIncome') > 0){
			calculateTaxPayableOnTotalInc('partBTI.netAgricultureIncomeOrOtherIncomeForRate','partBTTI.computationOfTaxLiability.taxPayableOnTI.rebateOnAgriInc', true);
		}else{
...
function calsurchargeOnAboveCrore(cgosIncome){
	...
	// for surcharge
				...
				if( rndOffNrsTen(totInc) > 10000000 ){
					var tempSurcharge = taxPayable.value  * 0.1 ;
...
try{
		...
		}else if(dueDate234A == '31/10/2015'){
			dueDate234A = '01/11/2015';
			
		}else if(dueDate234A == '30/11/2015'){
			dueDate234A = '01/12/2015';
		}
        ...
        intrst234C = calculate234cIntrst(TDS, TCS, [slab1, slab2, slab3, slab4], cgosIncome);
...
function isMarginalRelfApplcbl(){
	...
	if(parseInt(surcharge.value) > 0){
		if(parseInt(surcharge.value * 0.1) != taxPybl.value){
			return true;
		}
	}
	return false;
...
function isMarginalRelfApplcblCheck(){
	...
	var taxPybl = document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.taxPayable')[0];
	if(parseInt(surcharge.value) > 0){
		if(parseInt(surcharge.value ) != parseInt(taxPybl.value* 0.1)){
			return true;
		}
	}
	return false;
...
function calculate234cIntrst(TDS, TCS, slabs, cgosIncome){
		...
		if(sec44AD>0 &&((coalesce(parseInt(document.getElementsByName('scheduleVIA.deductUndChapVIA.totalDeductionPartc')[0].value, 10))+
				(coalesce(parseInt(document.getElementsByName('schedule10A.deductSEZ.dedUs10Detail.totalDedUs10Sub')[0].value, 10)))+
				(coalesce(parseInt(document.getElementsByName('schedule10AA.deductSEZ.dedUs10Detail.totalDedUs10Sub')[0].value, 10))) == 0))){
			Taxpayable44AD=calculateTaxPayable(normalBalIncome+parseInt(zeroOrMore( parseInt(cgosIncome.cgInc.stcg.prctgAr, 10)))-sec44AD)+parseInt(document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.taxAtSpecialRates')[0].value);
			Surcharge44AD=(parseInt(coalesceSetRet('partBTTI.computationOfTaxLiability.taxPayableOnTI.surcharge'))*((totalInc-sec44AD)/totalInc));
		}else{
			Surcharge44AD=document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.surcharge')[0].value;
			Taxpayable44AD=document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.taxPayable')[0].value;
		}

		if(parseInt(coalesceSetRet('partBTTI.computationOfTaxLiability.taxPayableOnTI.surcharge'))>0){
	...
	//App Rate
		...
		var surcharge;
		surcharge = parseInt(coalesceSetRet('partBTTI.computationOfTaxLiability.taxPayableOnTI.surcharge')) ;
		
		var dtaa = document.getElementsByName('scheduleSI.splCodeRateTax[1].splRateIncTax')[0].value;
		var rebate87A = document.getElementsByName('partBTTI.computationOfTaxLiability.taxPayableOnTI.rebate87A')[0].value;
		
		if(totalCG>0){
			taxOnstcgOthers1 = (((calculateSlabbedTax(taxOnstcgOthers1)  - rebate87A) + parseInt(coalesce(dtaa))) * appRateSurcharge ) ;
			taxOnstcgOthers2 = (((calculateSlabbedTax(taxOnstcgOthers2) - rebate87A) + parseInt(coalesce(dtaa))) * appRateSurcharge ) ;
			taxOnstcgOthers3 = calculateSlabbedTax(taxOnstcgOthers3);
			if ((calculateSlabbedTax(taxOnstcgOthers4) - parseInt(taxOnstcgOthers3) - rebate87A - parseInt(coalesce(dtaa))) > 0) {
			taxOnstcgOthers4 = ((zeroOrMore(calculateSlabbedTax(taxOnstcgOthers4) - parseInt(taxOnstcgOthers3) - rebate87A - parseInt(coalesce(dtaa)))))  ;
			} else{
			taxOnstcgOthers4 = 0;
			}
			taxOnstcgOthers3 = (((taxOnstcgOthers3  - rebate87A) + parseInt(coalesce(dtaa))) * appRateSurcharge);
		}else{
			taxOnstcgOthers1 = (calculateSlabbedTax(taxOnstcgOthers1) - rebate87A + parseInt(coalesce(dtaa)))  ;
			taxOnstcgOthers2 = (calculateSlabbedTax(taxOnstcgOthers2) - rebate87A + parseInt(coalesce(dtaa)) );
			taxOnstcgOthers3 = calculateSlabbedTax(taxOnstcgOthers3);
			if ((calculateSlabbedTax(taxOnstcgOthers4) - parseInt(taxOnstcgOthers3) - rebate87A - parseInt(coalesce(dtaa))) > 0) {
			taxOnstcgOthers4 = (zeroOrMore(calculateSlabbedTax(taxOnstcgOthers4) - parseInt(taxOnstcgOthers3) - rebate87A - parseInt(coalesce(dtaa))))  ;
			} else{
			...
			taxOnstcgOthers3 = (taxOnstcgOthers3  - rebate87A + parseInt(coalesce(dtaa)) ) ;
		}
		...
		totalUpto15Of9 = taxOnltcgProviso1 + taxOnltcgNonProviso1 + taxOnstcg111A1 + taxOnstcgOthers1 + shortTermUnder30Per1;
		 ...
		 if(marginal1 ||(sec44AD>0&&((coalesce(parseInt(document.getElementsByName('scheduleVIA.deductUndChapVIA.totalDeductionPartc')[0].value, 10))+
			...
			if(Surcharge44AD>0){
			totalUpto15Of9 = (parseInt(totalUpto15Of9) + parseInt(actualTax[0])*0.1) * 1.03;
			 totalUp16Of9To15Of12 = parseInt(totalUpto15Of9) + (parseInt(actualTax[1]) + actualTax[1]*0.1) * 1.03;
			 totalUp16Of12To15Of3 = parseInt(totalUp16Of9To15Of12) + (parseInt(actualTax[2]) + actualTax[2]*0.1) * 1.03;
			 totalUp16Of3To31Of3 = (parseInt(actualTax[3]) + actualTax[3]*0.1) * 1.03;
			}
			else{
		...
		if((sec44AD>0 &&((coalesce(parseInt(document.getElementsByName('scheduleVIA.deductUndChapVIA.totalDeductionPartc')[0].value, 10))+
				(coalesce(parseInt(document.getElementsByName('schedule10A.deductSEZ.dedUs10Detail.totalDedUs10Sub')[0].value, 10)))+
				(coalesce(parseInt(document.getElementsByName('schedule10AA.deductSEZ.dedUs10Detail.totalDedUs10Sub')[0].value, 10))) == 0))) && ((parseInt((Taxpayable44AD-othExemptions-rebate87A)*1.03)- parseInt(TDS ,10) - parseInt(TCS ,10))  <  parseInt('10000',10))){
...
try{
	...
	for(var i = 0; i < tab.rows.length-3; i++) {
        ...
        balancePLOthThanSpecBus.value = eval(parseInt(profBfrTaxPL.value ,10)) - eval(parseInt(netPLFromSpecBus.value ,10)) - eval(parseInt(netProfLossSpecifiedBus.value,10)) - eval(parseInt(incRecCredPLOthHeadsSalary.value,10)) - eval(parseInt(incRecCredPLOthHeadsHouseProperty.value,10))- eval(parseInt(incRecCredPLOthHeadsCapitalGains.value,10))- eval(parseInt(incRecCredPLOthHeadsOthSources.value,10)) - eval(parseInt(plUs44SChapXIIG.value,10)) - eval(parseInt(totExempInc.value,10));
        // 9. Total (7 + 8)
        totExpDebPL.value =  eval(parseInt(expDebToPLOthHeadsSalary.value,10)) + eval(parseInt(expDebToPLOthHeadsHouseProperty.value,10)) +eval(parseInt(expDebToPLOthHeadsCapitalGains.value,10)) +eval(parseInt(expDebToPLOthHeadsOthSources.value,10)) + eval(parseInt(expDebToPLExemptInc.value,10)) ;
...
function calcTotScheduleAMTC(){
    try{
        var totAmtcCredGrossPY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[0].amtCreditFwd')[0];
        var totAmtcCredSetOffPY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[0].amtCreditSetOfEy')[0];
        var totAmtcCredBFPY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[0].amtCreditBalBroughtFwd')[0];
        var totAmtcCredUtilPY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[0].amtCreditUtilized')[0];
        var totBalAmtcCredCFPY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[0].balAmtCreditCarryFwd')[0];
        ...
        var totAmtcCredGrossCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[1].amtCreditFwd')[0];
        var totAmtcCredBFCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[1].amtCreditBalBroughtFwd')[0];
        var totAmtcCredUtilCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[1].amtCreditUtilized')[0];
        var totBalAmtcCredCFCY = document.getElementsByName('scheduleAMTC.scheduleAMTCUtil.prevAY[1].balAmtCreditCarryFwd')[0];
     
       totAmtcCredBFPY.value = eval(parseInt(coalesce(totAmtcCredGrossPY.value),10))-eval(parseInt(coalesce(totAmtcCredSetOffPY.value),10));
      
        totBalAmtcCredCFPY.value = eval(parseInt(coalesce(totAmtcCredBFPY.value),10))-eval(parseInt(coalesce(totAmtcCredUtilPY.value),10));
        ...
        document.getElementsByName('itrScheduleAMTC.totAMTGross')[0].value=eval(parseInt(coalesce(totAmtcCredGross.value,10))) + eval(coalesce(parseInt(totAmtcCredGrossPY.value,10))) + eval(coalesce(parseInt(totAmtcCredGrossCY.value,10)));
        document.getElementsByName('itrScheduleAMTC.totSetOffEys')[0].value=eval(parseInt(coalesce(totAmtcCredSetOffPY.value,10)));
        document.getElementsByName('itrScheduleAMTC.totBalBF')[0].value=eval(parseInt(coalesce(totAmtcCredBFPY.value,10))) +eval(parseInt(coalesce(totAmtcCredBFCY.value,10))) ;
        
        document.getElementsByName('itrScheduleAMTC.totBalAMTCreditCF')[0].value=eval(parseInt(coalesce(totBalAmtcCredCFPY.value,10))) + eval(parseInt(coalesce(totBalAmtcCredCFCY.value,10)))+ eval(coalesce(parseInt(balAmtcCreditCarriedFwdCY.value,10)));
...
function validateAMTC(){
     ...
     if(parseInt(coalesce(totAmtcCredSetOffPY.value),10) > parseInt(coalesce(totAmtcCredGrossPY.value),10)){
 		...
 		totAmtcCredSetOffPY.value=parseInt(0,10);
 		totAmtcCredBFPY.value=parseInt(0,10);
     }
     
     
     if(parseInt(coalesce(totAmtcCredUtilPY.value),10) > parseInt(coalesce(totAmtcCredBFPY.value),10)){
 		...
 		totBalAmtcCredCFPY.value=parseInt(0,10);
 		totAmtcCredUtilPY.value=parseInt(0,10);
 		document.getElementsByName('itrScheduleAMTC.totAmtCreditUtilisedCY')[0].value=eval(parseInt(coalesce(totAmtcCredUtilPY.value),10))+eval(parseInt(coalesce(totAmtcCredUtilCY.value),10));
     }
...
function caclDedSchVIA(){
	...
	if(parseInt(gtiLimit,10)> eval('100000') ){		
		if( parseInt(coalesce(sec80CCC.value),10) > eval('100000')) {
			sec80CCCSysCal.value = '100000' ;
		} else {
	...
	if(parseInt(coalesce(salaries.value)) == parseInt('0' , 10)){
			if(gtiUppLimit > parseInt('100000' ,10)){
				if( parseInt(coalesce(sec80CCDi.value),10) > eval('100000')) {
					sec80CCDiSysCal.value = '100000' ;
				} else {
	...
	if(status =='I'){	
		 ...
		 var age = calcAge();
		if(status =='I'){
			if((resStatus== 'RES' || resStatus== 'NOR') && age > eval('59')){
				
				if(gtiLimit > parseInt('40000' ,10)){
				  if( parseInt(coalesce(sec80D.value),10) > eval('40000')){
					 sec80DSysCal.value = '40000';
				  } else {
				...
				if(gtiLimit > parseInt('35000' ,10)){
				if( parseInt(coalesce(sec80D.value),10) > eval('35000')){
					
					sec80DSysCal.value = '35000';
				} else {
					sec80DSysCal.value = parseInt(coalesce(sec80D.value),10);
				}
				}else{
				...
				if(gtiLimit > parseInt('20000' ,10)){
			if( parseInt(coalesce(sec80D.value),10) > eval('20000')){
					
					sec80DSysCal.value = '20000';
				} else {
					sec80DSysCal.value = parseInt(coalesce(sec80D.value),10);
				}
				}else{
...
// CHECK FOR 80DD	
	if(resStatus =='RES' || resStatus =='NOR'){
		...
		if(parseInt(gtiLimit,10)> eval('100000') ){
			...
			if( parseInt(coalesce(sec80DD.value),10) > eval('100000')){
				sec80DDSysCal.value= '100000';
			}else{
...
//CHECK FOR 80DDB
	if(resStatus =='RES' || resStatus =='NOR'){
		...
		if(parseInt(gtiLimit,10)> eval('60000') ){
		
		if(parseInt(coalesce(sec80DDB.value) ,10) > eval('60000')){
	...
	if(status =='I'){
		...
		if(parseInt(coalesce(sec80EE.value) ,10) > eval('100000')){
				sec80EESysCal.value = '100000';
			} else {
...
//80QQB
	...
	if(status =='I'){
		if(resStatus=='RES' || resStatus=='NOR'){	
			if(parseInt(coalesce(gtiLimit),10)> eval('100000') ){
				if( parseInt(coalesce(sec80U.value),10) > eval('100000')) {
					sec80USysCal.value = '100000' ;
				} else {
	...
	sumDeductionsWithout80GG(sec80CSysCal,sec80CCCSysCal,sec80CCDiSysCal,sec80CCDiiSysCal,
			sec80DSysCal,sec80DDSysCal,sec80DDBSysCal,sec80ESysCal,sec80EESysCal,sec80GSysCal,sec80GGCSysCal,
	...
	if(status =='I' ){		
		if(eval(oneFrthTI) < eval('24000')) {
			...
			if(eval(sec80GG.value) > eval('24000')){
				sec80GGSysCal.value = eval('24000');
			}else {
				sec80GGSysCal.value = sec80GG.value;
			}
		}
	...
	sumDeductions(sec80CSysCal,sec80CCCSysCal,sec80CCDiSysCal,sec80CCDiiSysCal,
			sec80DSysCal,sec80DDSysCal,sec80DDBSysCal,sec80ESysCal,sec80EESysCal,sec80GSysCal,sec80GGSysCal,sec80GGCSysCal,
			sec80IASysCal,sec80IABSysCal,sec80IBSysCal,sec80ICSysCal,sec80IDSysCal,sec80JJASysCal,
			sec80USysCal,sec80CCGSysCal,sec80RRBSysCal,sec80QQBSysCal,sec80TTASysCal);
...
function sumDeductions(sec80CSysCalc,sec80CCCSysCalc,sec80CCDiSysCalc,sec80CCDiiSysCalc,
...
sec80CCDiSysCalc.value 		= coalesce(sec80CCDiSysCalc.value); 				sec80CCDiiSysCalc.value 	= coalesce(sec80CCDiiSysCalc.value);
...
sec80GGSysCalc.value 			= coalesce(sec80GGSysCalc.value); 				sec80JJASysCal.value=   coalesce(sec80JJASysCal.value);
...
dedPartB.value=eval(sec80CSysCalc.value) + eval(sec80CCCSysCalc.value) + eval(sec80CCDiiSysCalc.value) +
...
dedPartC.value=  eval(sec80IASysCal.value)+ eval(sec80IABSysCal.value)+eval(sec80ICSysCal.value)+ eval(sec80IDSysCal.value)+ eval(sec80IBSysCal.value)+
                eval(sec80JJASysCal.value)+eval(sec80RRBSysCalc.value)+ eval(sec80QQBSysCalc.value);
...
function sumDeductionsWithout80GG(sec80CSysCalc,sec80CCCSysCalc,sec80CCDiSysCalc,sec80CCDiiSysCalc,
...
var dedVIA = document.getElementsByName('scheduleVIA.deductUndChapVIA.totalChapVIADeductions')[0];
var temp2 = eval(sec80CSysCalc.value) + eval(sec80CCCSysCalc.value) + eval(sec80CCDiiSysCalc.value) +
			eval(sec80CCDiSysCalc.value) + eval(sec80DSysCalc.value) +
			eval(sec80DDSysCalc.value) + eval(sec80DDBSysCalc.value) + eval(sec80ESysCalc.value) +  eval(sec80EESysCalc.value) +
			eval(sec80GSysCalc.value) + eval(sec80JJASysCal.value)+
			...
			eval(sec80IASysCal.value)+ eval(sec80IABSysCal.value)+eval(sec80ICSysCal.value)+ eval(sec80IDSysCal.value)+ eval(sec80IBSysCal.value)
			;
...
function populateAMT(){
	...
	//3=1+2c
	adjustedUnderSec115JC.value=eval(parseInt(coalesce(amtTotIncome.value),10))+eval(parseInt(coalesce(sec115JCTotal.value),10));
	//4
...
var hpIncome1 = Math.abs(coalescePath('schedule5A2014.hPHeadIncome.incRecvdUndHead'));
var busHeadIncome1 = Math.abs(coalescePath('schedule5A2014.hPHeadIncome.amtApprndOfSpouse'));
...
var hpIncome2 = Math.abs(coalescePath('schedule5A2014.hPHeadIncome.amtTDSDeducted'));
var busHeadIncome2 = Math.abs(coalescePath('schedule5A2014.hPHeadIncome.tDSApprndOfSpouse'));
...
var hpIncome3 = Math.abs(coalescePath('schedule5A2014.busHeadIncome.incRecvdUndHead'));
var busHeadIncome3 = Math.abs(coalescePath('schedule5A2014.busHeadIncome.amtApprndOfSpouse'));
...
var hpIncome4 = Math.abs(coalescePath('schedule5A2014.busHeadIncome.amtTDSDeducted'));
var busHeadIncome4 = Math.abs(coalescePath('schedule5A2014.busHeadIncome.tDSApprndOfSpouse'));
...
var hpIncome5 = Math.abs(coalescePath('schedule5A2014.capGainHeadIncome.incRecvdUndHead'));
var busHeadIncome5 = Math.abs(coalescePath('schedule5A2014.capGainHeadIncome.amtApprndOfSpouse'));
...
var hpIncome6 = Math.abs(coalescePath('schedule5A2014.capGainHeadIncome.amtTDSDeducted'));
var busHeadIncome6 = Math.abs(coalescePath('schedule5A2014.capGainHeadIncome.tDSApprndOfSpouse'));
...
var hpIncome7 = Math.abs(coalescePath('schedule5A2014.otherSourcesHeadIncome.incRecvdUndHead'));
var busHeadIncome7 = Math.abs(coalescePath('schedule5A2014.otherSourcesHeadIncome.amtApprndOfSpouse'));
...
var hpIncome8 = Math.abs(coalescePath('schedule5A2014.otherSourcesHeadIncome.amtTDSDeducted'));
var busHeadIncome8 = Math.abs(coalescePath('schedule5A2014.otherSourcesHeadIncome.tDSApprndOfSpouse'));
...
function adjustForm(){
		...
		enableField('partAGEN1.filingStatus.residentialStatus',['RES','NOR',''],'partBTTI.assetOutIndiaFlag');
...
function onStatusChange(){
	...
	if(status.value == 'I' || status.value == ''){
		if(gender.value=='X') {
...
function validateCGSecwiseDed(){
	var arr = {'54':0,'54B':0,'54EC':0,'54F':0,'54GB':0,'115F':0,'54D':0,'54G':0,'54GA':0};
	var arrDed = {'54':0,'54B':0,'54EC':0,'54F':0,'54GB':0,'115F':0,'54D':0,'54G':0,'54GA':0};
...
function mandateA6()
{
	...
	var valueSTCG= valueA13+valueAb4+valueA1d+valueA2ia+valueA2b4+valueA2d+valueA3a+valueA3b+valueA4a+valueA44+valueA4d+valueA5a+valueA54+valueA5d+valueA6+valueA8;
...
function validateSTCGSectionWiseDTAA() {
	var arr = {'A1e':0,'A2c':0,'A3e_111A':0,'A3e_115AD':0,'A4a':0,'A4b':0,'A5e':0,'A6f':0,'A7':0,'A8':0};
	var tab = document.getElementById('scheduleStcgDtaa');
	  ...
	  for(var i=0; i<rowCount; i++) {
	   ...
	   if(itemIncluded == 'A1e') {
		...
		} else if(itemIncluded == 'A6f') {
		       arr['A6f'] = parseInt(arr['A6f'],10) +  parseInt(coalesce(amount),10);
		...
		} else if(itemIncluded == 'A8') {
		       arr['A8'] = parseInt(arr['A8'],10) +  parseInt(coalesce(amount),10);
		}
	  }
	...
	if(dtaaSection.indexOf('A6f') != -1 && arr['A6f'] > 0 && fullConsiderationA6f.value == 0) {
	...
	var deprAssest = document.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.amtDeemedCGDepAssets')[0];
	if(dtaaSection.indexOf('A8') != -1 && arr['A8'] > 0 && deprAssest.value == 0) {
		j.setFieldError('scheduleCGPost45.shortTermCapGainPost45.amtDeemedCGDepAssets','A8 value should not be zero as there is an entry in DTAA table.');
...
function setValuesCG54B() {
	...
	if (yearConstant == "" && amountUtilized == "" && amountUnUtilized == "") {
		...
		document.getElementsByName('scheduleCGPost45.longTermCapGainPost45.unUtilizedCapGain54B.year')[0].value = '2012-13';
	}
	...
	if (yearConstantStcg == "" && amountUtilizedStcg == "" && amountUnUtilizedStcg == "") {
		...
		document.getElementsByName('scheduleCGPost45.shortTermCapGainPost45.unUtilizedCapGain54B.year')[0].value = '2012-13';
	}
...
// Count no of Rows Filled for Schedule BA.
